...
plugin = Plugin(
    ...
    description=(
        "This QIIME 2 plugin supports metrics for calculating "
...
plugin.pipelines.register_function(
    ...
    parameters={
        "metric": Str
        % Choices(beta.METRICS["PHYLO"]["IMPL"] | beta.METRICS["PHYLO"]["UNIMPL"]),
        "threads": Threads,
    ...
    description=(
        "Computes a user-specified phylogenetic beta diversity metric"
...
plugin.pipelines.register_function(
    ...
    parameters={
        "metric": Str
        % Choices(
            beta.METRICS["NONPHYLO"]["IMPL"] | beta.METRICS["NONPHYLO"]["UNIMPL"]
        ),
    ...
    parameter_descriptions={
        "metric": "The beta diversity metric to be computed.",
        "pseudocount": (
            "A pseudocount to handle zeros for compositional "
    ...
    name="Beta diversity",
    description=(
        "Computes a user-specified beta diversity metric for all "
...
plugin.pipelines.register_function(
    ...
    parameters={
        "metric": Str
        % Choices(alpha.METRICS["PHYLO"]["IMPL"] | alpha.METRICS["PHYLO"]["UNIMPL"])
    },
    ...
    parameter_descriptions={"metric": "The alpha diversity metric to be computed."},
    output_descriptions={
        "alpha_diversity": "Vector containing per-sample alpha diversities."
    },
    name="Alpha diversity (phylogenetic)",
    description=(
        "Computes a user-specified phylogenetic alpha diversity "
...
plugin.pipelines.register_function(
    ...
    parameters={
        "metric": Str
        % Choices(
            alpha.METRICS["NONPHYLO"]["IMPL"] | alpha.METRICS["NONPHYLO"]["UNIMPL"]
        )
    },
    outputs=[("alpha_diversity", SampleData[AlphaDiversity])],
    input_descriptions={
    ...
    description=(
        "Computes a user-specified alpha diversity metric for all "
...
plugin.methods.register_function(
    ...
    parameters={"number_of_dimensions": Int % Range(1, None)},
    ...
    citations=[citations["legendrelegendre"], citations["halko2011"]],
)

plugin.methods.register_function(
    function=q2_diversity.pcoa_biplot,
    inputs={"pcoa": PCoAResults, "features": FeatureTable[RelativeFrequency]},
    parameters={},
...
plugin.methods.register_function(
    ...
    input_descriptions={
        "distance_matrix": ("The distance matrix on which t-SNE should be " "computed.")
    },
    parameter_descriptions={
        "number_of_dimensions": "Dimensions to reduce the distance matrix to.",
...
plugin.methods.register_function(
    ...
    input_descriptions={
        "distance_matrix": ("The distance matrix on which UMAP should be " "computed.")
    },
    parameter_descriptions={
        "number_of_dimensions": "Dimensions to reduce the distance matrix to.",
...
plugin.methods.register_function(
    ...
    output_descriptions={
        "transformed": (
            "The 'other' ordination transformed into the space of "
            "the reference ordination."
        ),
    },
    name="Partial Procrustes",
    description=(
        "Transform one ordination into another, using paired samples "
...
plugin.pipelines.register_function(
    ...
    output_descriptions={
        ...
        "observed_features_vector": "Vector of Observed Features values by " "sample.",
        ...
        "unweighted_unifrac_pcoa_results": "PCoA matrix computed from unweighted UniFrac distances between "
        "samples.",
        "weighted_unifrac_pcoa_results": "PCoA matrix computed from weighted UniFrac distances between "
        "samples.",
        "jaccard_pcoa_results": "PCoA matrix computed from Jaccard distances between "
        "samples.",
        "bray_curtis_pcoa_results": "PCoA matrix computed from Bray-Curtis distances between "
        "samples.",
        "unweighted_unifrac_emperor": "Emperor plot of the PCoA matrix computed from unweighted"
        " UniFrac.",
        "weighted_unifrac_emperor": "Emperor plot of the PCoA matrix computed from weighted UniFrac.",
        "jaccard_emperor": "Emperor plot of the PCoA matrix computed from Jaccard.",
        "bray_curtis_emperor": "Emperor plot of the PCoA matrix computed from Bray-Curtis.",
    },
...
plugin.pipelines.register_function(
    ...
    output_descriptions={
        ...
        "jaccard_distance_matrix": "Matrix of Jaccard distances between pairs of samples.",
        "bray_curtis_distance_matrix": "Matrix of Bray-Curtis distances between pairs of samples.",
        "jaccard_pcoa_results": "PCoA matrix computed from Jaccard distances between samples.",
        "bray_curtis_pcoa_results": "PCoA matrix computed from Bray-Curtis distances between samples.",
        "jaccard_emperor": "Emperor plot of the PCoA matrix computed from Jaccard.",
        "bray_curtis_emperor": "Emperor plot of the PCoA matrix computed from Bray-Curtis.",
    },
...
plugin.pipelines.register_function(
    ...
    description=(
        "Create a distance matrix from a numeric metadata column and "
...
plugin.methods.register_function(
    ...
    inputs={"distance_matrix": DistanceMatrix},
    parameters={"metadata": Metadata, "where": Str, "exclude_ids": Bool},
    outputs=[("filtered_distance_matrix", DistanceMatrix)],
    ...
    input_descriptions={"distance_matrix": "Distance matrix to filter by sample."},
    parameter_descriptions={
...
plugin.methods.register_function(
    ...
    parameters={"metadata": Metadata, "exclude_ids": Bool, "where": Str},
    outputs=[("filtered_alpha_diversity", SampleData[AlphaDiversity])],
...
plugin.visualizers.register_function(
    ...
    examples={
        "alpha_group_significance_faith_pd": ex.alpha_group_significance_faith_pd
    },
)
...
plugin.visualizers.register_function(
    ...
    name="bioenv",
    description=(
        "Find the subsets of variables in metadata whose Euclidean "
...
beta_group_significance_methods = list(
    q2_diversity._beta._visualizer._beta_group_significance_fns
)
...
plugin.visualizers.register_function(
    ...
    inputs={"distance_matrix": DistanceMatrix},
    parameters={
        "method": Str % Choices(beta_group_significance_methods),
        "permutations": Int,
    ...
    description=(
        "Determine whether groups of samples are significantly "
...
plugin.visualizers.register_function(
    ...
    inputs={"dm1": DistanceMatrix, "dm2": DistanceMatrix},
    parameters={
        "permutations": Int % Range(0, None),
        "method": Str % Choices(["spearman", "pearson"]),
        "intersect_ids": Bool,
...
alpha_correlation_methods = list(q2_diversity._alpha._visualizer._alpha_correlation_fns)

plugin.visualizers.register_function(
    function=q2_diversity.alpha_correlation,
    inputs={"alpha_diversity": SampleData[AlphaDiversity]},
    parameters={
        "method": Str % Choices(alpha_correlation_methods),
        "metadata": Metadata,
        "intersect_ids": Bool,
    },
    input_descriptions={
    ...
    description=(
        "Determine whether numeric sample metadata columns are "
...
plugin.visualizers.register_function(
    ...
    parameter_descriptions={
        ...
        "steps": (
            "The number of rarefaction depths to include "
    ...
    description=(
        "Generate interactive alpha rarefaction curves by computing "
...
plugin.visualizers.register_function(
    ...
    parameter_descriptions={
        ...
        "metadata": "The sample metadata used for the Emperor jackknifed PCoA " "plot.",
        ...
        "distance matrices.",
        "color_scheme": "The matplotlib color scheme to generate the heatmap " "with.",
    },
...
plugin.visualizers.register_function(
    ...
    citations=[
        citations["anderson2001new"],
        citations["Oksanen2018"],
